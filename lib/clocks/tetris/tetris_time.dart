...
class _TetrisClockState extends State<TetrisClock> {
  ...
  late var tetrisNumberAnimationData;
  ...
  */
  ...
  Future<void> initTetrisData() async {
    ...
    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/splash_screen.json");
  ...
  Future<void> initTetrisData2() async {
    ...
    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/tetris_blocks.json");
    tetrisBlocks = jsonDecode(jsonData);

    tetrisNumberAnimationData = [];
    for (int i = 0; i < 10; i++) {
      jsonData = await DefaultAssetBundle.of(context)
          .loadString("packages/clocks/assets/tetris/numbers/tetris_number$i.json");
    ...
    // Weather animations
    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/sun.json");
    weatherSun = jsonDecode(jsonData);

    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/cloudy.json");
    weatherCloudy = jsonDecode(jsonData);

    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/rainy.json");
    weatherRainy = jsonDecode(jsonData);

    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/thunderstorm.json");
    weatherThunderstorm = jsonDecode(jsonData);

    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/snowy.json");
    weatherSnowy = jsonDecode(jsonData);

    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/foggy.json");
    weatherFoggy = jsonDecode(jsonData);

    jsonData = await DefaultAssetBundle.of(context)
        .loadString("packages/clocks/assets/tetris/weather/windy.json");
  ...
  List<TetrisNumber?> updateTetrisBlocks(
      int block, int number, int lastNumber) {
...
class TetrisNumber {
  ...
  List getBlockPosData() {
    List blockData = [];
    for (List block in blocks as List<dynamic>) {
  ...
  */
  ...
  void initLastBlockData() {
    ...
    if (blockPos < tetrisCols) {
      if (blocks != null) {
        for (List block in blocks as Iterable<List<dynamic>>) {
  ...
  void updateLastBlockData() {
    ...
    if (blockPos < tetrisCols && blockPos > 0) {
      for (List lastBlockElement in lastBlockData as List<dynamic>) {
